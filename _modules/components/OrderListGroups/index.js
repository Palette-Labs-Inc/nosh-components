...
export const OrderListGroups = props => {
  ...
  const getOrders = async ({
    ...
    if (filtered?.driver) {
      options.query.where.push({
        attribute: 'driver_id',
        value: filtered?.driver
      });
    }
    if (filtered?.driver_groups) {
      options.query.where.push({
        attribute: 'driver_id',
        value: filtered?.driver_groups?.drivers
      });
    }
    ...
    const functionFetch = asDashboard ? ordering.setAccessToken(accessToken).orders().asDashboard() : ordering.setAccessToken(accessToken).orders();
  ...
  const getControls = async () => {
    try {
      ...
      const obj = {
        ...controlsState,
        loading: false,
        paymethods: result?.paymethods?.filter(p => customPaymethods?.includes(p.name))?.map(pay => pay.id),
  ...
  const loadOrders = async ({
    ...
    try {
      ...
      const _ordersCleaned = error ? newFetch || newFetchCurrent ? [] : sortOrders(ordersGroup[currentTabSelected]?.orders) : newFetch || newFetchCurrent ? sortOrders(result) : sortOrders(ordersGroup[currentTabSelected]?.orders.concat(result));
      setOrdersGroup({
        ...ordersGroup,
        [currentTabSelected]: {
          ...ordersGroup[currentTabSelected],
  ...
  const loadMoreOrders = async () => {
    ...
    try {
      ...
      const _ordersCleaned = error ? sortOrders(ordersGroup[currentTabSelected]?.orders) : sortOrders(ordersGroup[currentTabSelected]?.orders?.concat(result));
      setOrdersGroup({
        ...ordersGroup,
        [currentTabSelected]: {
          ...ordersGroup[currentTabSelected],
  ...
  const deleteOrders = async orderIds => {
    try {
      ...
      if (orderIds.length === 1) {
        ...
        } = await ordering.setAccessToken(accessToken).orders(orderIds[0]).delete();
        errorState.push({
          error,
          id: orderIds[0]
        });
        ...
        for (const id of orderIds) {
          ...
          } = await ordering.setAccessToken(accessToken).orders(id).delete();
          errorState.push({
            error: multiError,
            id
          });
      ...
      setOrdersGroup({
        ...
        [currentTabSelected]: {
          ...ordersGroup[currentTabSelected],
          orders: idsDeleted.length ? sortOrders(ordersGroup[currentTabSelected]?.orders?.filter(order => !idsDeleted.includes(order.id))) : sortOrders(ordersGroup[currentTabSelected]?.orders)
        }
      });
    } catch (err) {
      if (err.constructor.name !== 'Cancel') {
    ...
    try {
      ...
      if (!error) {
        setlogisticOrders({
          ...logisticOrders,
          loading: false,
          orders: result.filter(order => !(order?.order_group && order?.order_group?.orders?.length === 0))
        });
        return;
      }
      setlogisticOrders({
  ...
  const formatOrdersGrouped = orders => {
    ...
    totalOrders = totalOrders?.map(item => {
      if (!item?.cart_group_id) return item;
      const groupIds = totalOrders.filter(o => o.cart_group_id === item?.cart_group_id);
      const _item = !ordersGroupids.includes(item?.cart_group_id) ? Object.entries({
        [item?.cart_group_id]: groupIds
      }) : '';
  ...
  const getStatusById = id => {
    ...
    const status = pending.includes(id) ? 'pending' : inProgress.includes(id) ? 'inProgress' : completed.includes(id) ? 'completed' : 'cancelled';
    return status;
  };
  const actionOrderToTab = (orderAux, status, type) => {
    ...
    if (type === 'update') {
      ...
      orders = type === 'add' ? [order, ...orderList] : orderList.filter(_order => _order.id !== order.id);
    }
    ...
    try {
      ...
      if (!error) {
        ...
        if (status === 1) {
          ...
          showToast(ToastType.Success, t('SPECIFIC_ORDER_ACCEPTED', 'Your accepted the order number _NUMBER_').replace('_NUMBER_', order?.order?.id ?? order?.id));
        } else {
          showToast(ToastType.Info, t('SPECIFIC_ORDER_REJECTED', 'Your rejected the order number _NUMBER_').replace('_NUMBER_', order?.order?.id ?? order?.id));
        }
        return;
      }
  ...
  const handleChangeOrderStatus = async (status, orderIds, body = {}) => {
    try {
      delete body.id;
      const bodyToSend = Object.keys(body || {}).length > 0 ? body : {
        status
      };
      const setOrderStatus = async id => {
        try {
          ...
          } = await ordering.setAccessToken(session?.token).orders(id).save({
            ...bodyToSend,
            id
          });
  ...
  const handleSendCustomerReview = async ({
    ...
    try {
      ...
      const result = await Promise.all(orderIds?.map(id => setCustomerReview({
        ...body,
        order_id: id,
        user_id: customerId
      })));
      if (result?.length) {
        ...
        showToast(ToastType.Success, t('ORDERS_SUCCESSFULLY_REVIEWED', 'Orders successfully reviewed'));
      }
      onClose && onClose();
      return result;
    } catch (err) {
  ...
  const getBusinessesIDs = async () => {
    ...
    try {
      ...
      } = await ordering.businesses().asDashboard().select(propsToFetch).get();
      if (!error) {
        const _businessIDs = result.length > 0 && result.map(({
          id
        }) => id);
  ...
  useEffect(() => {
    ...
    const handleUpdateOrder = order => {
      ...
      if (!orderFound) {
        if (!order?.products || !order?.summary || typeof order?.status !== 'number' || !order?.customer || !order?.business || !order?.paymethod) {
          return;
        }
        delete order.total;
        delete order.subtotal;
        const currentFilter = ordersGroup[getStatusById(order?.status) ?? '']?.currentFilter;
        !currentFilter.includes(order.status) ? actionOrderToTab(order, getStatusById(order?.status), 'remove') : actionOrderToTab(order, getStatusById(order?.status), 'add');
        return;
      }
      if (orderFound.id === order.id && orderFound?.driver?.id !== order?.driver?.id && session?.user?.level === 4) {
      ...
      if (!order?.status && order?.status !== 0) {
        ...
        if (newOrderStatus !== currentOrderStatus) {
          ...
          if (currentFilter.includes(orderFound.status) && total !== null) {
            actionOrderToTab(orderFound, newOrderStatus, 'add');
          }
        } else {
          !currentFilter.includes(orderFound.status) ? actionOrderToTab(orderFound, newOrderStatus, 'remove') : actionOrderToTab(orderFound, newOrderStatus, 'update');
        }
      }
    };
    const handleAddNewOrder = order => {
      events.emit('order_added', order);
      showToast(ToastType.Info, t('SPECIFIC_ORDER_ORDERED', 'Order _NUMBER_ has been ordered').replace('_NUMBER_', order.id), 1000);
      const status = getStatusById(order?.status) ?? '';
  ...
  const handleAddAssignRequest = useCallback(order => {
    setlogisticOrders({
      ...logisticOrders,
      orders: sortOrders([...logisticOrders?.orders, order])
    });
    showToast(ToastType.Info, t('SPECIFIC_LOGISTIC_ORDER_ORDERED', 'Logisitc order _NUMBER_ has been ordered').replace('_NUMBER_', order?.order?.id ?? order.id), 1000);
  }, []);
  const handleDeleteAssignRequest = useCallback(order => {
    setlogisticOrders(prevState => ({
      ...prevState,
      orders: prevState?.orders?.some(_order => _order?.id === order?.id) ? sortOrders([...prevState?.orders?.filter(_order => _order?.id !== order?.id), {
        ...prevState?.orders?.find(_order => _order?.id === order?.id),
        expired: true
      }]) : sortOrders(prevState?.orders)
    }));
  }, []);
  const handleUpdateAssignRequest = useCallback(order => {
    setlogisticOrders(prevState => ({
      ...prevState,
      orders: prevState?.orders?.some(_order => _order?.id === order?.id) ? sortOrders([...prevState?.orders?.filter(_order => _order?.id !== order?.id), {
        ...prevState?.orders?.find(_order => _order?.id === order?.id),
    ...
    showToast(ToastType.Info, t('SPECIFIC_LOGISTIC_ORDER_UPDATED', 'Your logisitc order number _NUMBER_ has updated').replace('_NUMBER_', order?.order?.id ?? order.id), 1000);
  }, []);
  useEffect(() => {
