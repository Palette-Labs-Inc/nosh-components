...
export const BusinessReviews = props => {
  ...
  const onChangeOption = (val = null) => {
    const reviews = val !== 'all' ? reviewsList.filter(review => review.total >= val && review.total < val + 1) : reviewsList;
  ...
  const onChangeReview = text => {
    const reviews = text !== '' ? reviewsList.filter(review => review.comment.toLowerCase().includes(text?.toLowerCase())) : reviewsList;
    setBusinessReviewsList({
      ...businessReviewsList,
      loading: false,
      reviews
  ...
  const getBusiness = async () => {
    try {
      ...
      } = await ordering.businesses(businessId).select(['reviews']).get({
        cancelToken: source
      });
