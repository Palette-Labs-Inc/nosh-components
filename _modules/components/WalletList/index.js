...
export const WalletList = props => {
  ...
  const [userLoyaltyLevel, setUserLoyaltyLevel] = useState({
    loading: true,
    error: null,
    loyaltyLevel: null
  });
  ...
  const getTransactions = async walletId => {
    ...
    try {
      ...
      const response = await fetch(`${ordering.root}/users/${user.id}/wallets/${walletId}/events?orderBy=-id`, {
        method: 'GET',
        headers: {
  ...
  const getUserLoyaltyLevel = async () => {
    try {
      ...
      } = await ordering.users(user.id).select(userProps).get();
      setUserLoyaltyLevel({
        ...userLoyaltyLevel,
        loading: false,
