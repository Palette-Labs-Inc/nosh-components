...
export const PaymentOptionWallet = props => {
  ...
  const getWallets = async () => {
    try {
      const response = await fetch(`${ordering.root}/users/${user.id}/wallets`, {
        method: 'GET',
        headers: {
      ...
      if (!loyaltyPlansState) {
        const reqLoyalty = await fetch(`${ordering.root}/loyalty_plans`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
  ...
  const selectWallet = async wallet => {
    try {
      const response = await fetch(`${ordering.root}/carts/${cart.uuid}/wallets`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const deletetWalletSelected = async wallet => {
    try {
      const response = await fetch(`${ordering.root}/carts/${cart.uuid}/wallets/${wallet.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
