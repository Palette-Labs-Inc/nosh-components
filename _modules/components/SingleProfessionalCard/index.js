...
export const SingleProfessionalCard = props => {
  ...
  const handleFavoriteProfessional = async (isAdd = false) => {
    ...
    try {
      ...
      const fetchEndpoint = isAdd ? `${ordering.root}/users/${user?.id}/favorite_users` : `${ordering.root}/users/${user.id}/favorite_users/${professional?.id}`;
      ...
      if (!content.error) {
        ...
        handleUpdateProfessionals && handleUpdateProfessionals(professional?.id, {
          favorite: isAdd
        });
