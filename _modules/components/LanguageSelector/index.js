...
export const LanguageSelector = props => {
  ...
  const [languagesState, setLanguageState] = useState({
    loading: !languages,
    languages,
    error: null
  });
  ...
  const loadLanguages = async () => {
    try {
      ...
      } = await ordering.languages().where([{
        attribute: 'enabled',
        value: true
      }]).get({
        cancelToken: source
  ...
  useEffect(() => {
    if (currentLanguage && languages?.length > 0) {
      ...
      const language = languageState?.language?.code ? languageState?.language : languagesState?.languages?.find(language => language.default);
      if (language) {
        setLanguage(language);
        setLanguageSelected(language);
      }
