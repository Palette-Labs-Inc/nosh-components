...
export const LoginForm = props => {
  ...
  const useLoginByEmail = useLoginByCellphone || useLoginOtpEmail || useLoginOtpCellphone ? configs?.email_password_login_enabled?.value === '1' : true;
          ...
          if (allowedLevels && allowedLevels?.length > 0) {
            ...
            if (!allowedLevels.includes(level)) {
              try {
                const {
                  content: logoutResp
                } = await ordering.setAccessToken(accessToken).users().logout();
                if (!logoutResp.error) {
                  logout();
                }
                setFormState({
  ...
  useEffect(() => {
    setIsReCaptchaEnable(props.isRecaptchaEnable && configs && Object.keys(configs).length > 0 && configs?.security_recaptcha_auth?.value === '1');
