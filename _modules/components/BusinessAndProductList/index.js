...
export const BusinessAndProductList = props => {
  ...
  const isMatchSearch = (name, description, price) => {
    ...
    return (searchValue ? name && name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(searchValue.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')) && isSearchByName : true) && (priceFilterValues?.min ? parseFloat(price) >= parseFloat(priceFilterValues?.min) : true) && (priceFilterValues?.max ? parseFloat(price) <= parseFloat(priceFilterValues?.max) : true) || (searchValue ? description && description.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(searchValue.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')) && isSearchByDescription : true) && (priceFilterValues?.min ? parseFloat(price) >= parseFloat(priceFilterValues?.min) : true) && (priceFilterValues?.max ? parseFloat(price) <= parseFloat(priceFilterValues?.max) : true);
  };
  ...
  const iterateCategories = categories => {
    return categories?.length > 0 && categories?.forEach(category => {
  ...
  const getProducts = async () => {
    ...
    if (categorySelected.id !== 'featured' && categorySelected.id !== null) {
      ...
      const productsFiltered = businessState?.business?.categories?.find(category => category.id === (isUseParentCategory ? parentCategory?.parent_category_id : categorySelected.id))?.products.filter(product => isUseParentCategory ? categoryFinded?.children?.some(cat => cat.category_id === product?.category_id) && isMatchSearch(product.name, product.description, product?.price) : isMatchSearch(product.name, product.description, product?.price));
      categoryState.products = productsFiltered || [];
    } else if (categorySelected.id === 'featured') {
      const productsFiltered = businessState?.business?.categories?.reduce((products, category) => [...products, ...category.products], []).filter(product => isFeaturedSearch(product));
      categoryState.products = productsFiltered || [];
    } else {
      let _categoriesCustom = null;
      if (avoidProductDuplicate) {
        const customCategories = ['favorites'];
        _categoriesCustom = businessState?.business?.categories?.filter(({
          id
      ...
      const productsFiltered = productsToFilter?.reduce((products, category) => [...products, ...category.products], []).filter(product => isMatchSearch(product.name, product.description, product?.price));
      categoryState.products = productsFiltered || [];
    }
  ...
  const getLazyProducts = async ({
    ...
    const promises = [];
    const functionFetch = categorySelected.id && categorySelected.id !== 'featured' ? ordering.businesses(businessState.business.id).categories(categorySelected.id).products() : !isUseParentCategory ? ordering.businesses(businessState.business.id).products() : !(where?.conditions?.length > 0) ? ordering.businesses(businessState.business.id).categories() : ordering.businesses(businessState.business.id).products();
    let productEndpoint = where?.conditions?.length > 0 ? functionFetch.parameters(parameters).where(where) : functionFetch.parameters(parameters);
    ...
    try {
      ...
      if (featuredRes?.content?.result?.length) {
        ...
        const featureState = {
          ...
          loading: false,
          products: newFetch ? [...featuredRes?.content?.result] : oldFeatured?.products?.concat(featuredRes?.content?.result)
        };
      ...
      if (categorySelected.id && categorySelected.id !== 'featured' || !isUseParentCategory) {
        ...
        const isFeatured = categoriesState.all.products.some(product => product.featured) || categoriesState?.featured?.products?.some(product => product.featured);
        setFeaturedProducts(isFeatured);
      }
      if (isUseParentCategory && (!categorySelected.id || categorySelected.id === 'featured')) {
        ...
        const paginationData = categorySelected.id === 'featured' ? categoriesState?.featured?.pagination ?? {} : curCategoryState?.pagination ?? {};
        const newcategoryState = {
          pagination: {
            ...paginationData,
            currentPage: categorySelected.id === 'featured' ? featuredRes?.content?.pagination?.current_page : pagination?.current_page,
            totalItems: categorySelected.id === 'featured' ? featuredRes?.content?.pagination?.total : pagination?.total,
            totalPages: categorySelected.id === 'featured' ? featuredRes?.content?.pagination?.total_pages : pagination?.total_pages
          },
          loading: false,
          products: categorySelected.id === 'featured' ? productsListFeatured : searchValue ? [...productsListFeatured, ...productsList] : [...productsListFeatured, ...curCategoryState.products.concat(productsList)]
        };
        categoriesState[categoryKey] = newcategoryState;
        setCategoryState({
          ...newcategoryState
    ...
    try {
      ...
      if (isUseParentCategory && (!categorySelected.id || categorySelected.id === 'featured')) {
        ...
        const newcategoryState = {
          ...
          loading: false,
          products: categorySelected.id === 'featured' ? productsListFeatured : [...productsListFeatured, ...(curCategoryState?.products?.concat(productsList) ?? [])]
        };
        categoriesState[categoryKey] = newcategoryState;
        categoryState = newcategoryState;
        setCategoryState({
  ...
  const getProduct = async () => {
    if (categoryId && productId && businessState.business.id || props.product?.businessId && props.product?.categoryId && props.product?.id) {
      try {
        ...
        } = await ordering.businesses(businessState.business.id || props.product?.businessId).categories(categoryId || props.product?.categoryId).products(productId || props.product?.id).parameters(parameters).get({
          cancelToken: source
        });
  ...
  const getBusiness = async () => {
    try {
      ...
      const parameters = {
        type: orderState.options?.type || 1,
        location: location ? `${location?.lat},${location?.lng}` : orderState.options?.address?.location ? `${orderState.options?.address?.location?.lat},${orderState.options?.address?.location?.lng}` : null
      };
      if (orderState.options?.moment && isValidMoment(orderState.options?.moment, 'YYYY-MM-DD HH:mm:ss')) {
        const moment = dayjs.utc(orderState.options?.moment, 'YYYY-MM-DD HH:mm:ss').local().unix();
        parameters.timestamp = moment;
      ...
      } = await ordering.businesses(slug).select(businessProps).parameters(parameters).get({
        cancelToken: source
      });
      ...
      if (menusProps && isGetMenus) {
        ...
        } = await ordering.businesses(result.id).menus().select(menusProps).get();
        data.menus = menus;
      }
  ...
  const multiRemoveProducts = async (unavailableProducts, carts) => {
    ...
    (await Promise.all(allPromise)) && setAlertState({
      open: true,
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, UIComponent && /*#__PURE__*/React.createElement(UIComponent, _extends({}, props, {
    ...
    updateProductModal: val => setProductModal({
      ...productModal,
      product: val
    }),
