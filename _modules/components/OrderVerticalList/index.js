...
export const OrderVerticalList = props => {
  ...
  const loadOrders = async ({
    ...
    if (!newFetch && ordersGroup.pagination.currentPage === ordersGroup.pagination?.totalPages && ordersGroup.pagination.total !== null) {
      return;
    }
    if (newFetch) {
      const options = {};
    ...
    try {
      ...
      ordersStatusArray.map(tab => {
        tabOptions[tab] = {
          orders: error ? newFetch ? [] : sortOrders(ordersGroup[tab].orders) : newFetch ? sortOrders(result.filter(order => ordersGroupStatus[tab].includes(order.status))) : sortOrders(ordersGroup[tab].orders.concat(result.filter(order => ordersGroupStatus[tab].includes(order.status))))
        };
      });
      setOrdersGroup({
        ...ordersGroup,
  ...
  const loadMoreOrders = async () => {
    try {
      ...
      ordersStatusArray.map(tab => {
        tabOptions[tab] = {
          orders: error ? sortOrders(ordersGroup[tab].orders) : sortOrders(ordersGroup[tab].orders.concat(result.filter(order => ordersGroupStatus[tab].includes(order.status))))
        };
      });
      setOrdersGroup({
        ...ordersGroup,
  ...
  const getStatusById = id => {
    ...
    const status = upcoming.includes(id) ? 'upcoming' : active.includes(id) ? 'active' : 'past';
    return status;
  };
