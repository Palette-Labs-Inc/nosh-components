...
export const GoogleLoginButton = props => {
  ...
  const initializeGoogleSignIn = () => {
    window.gapi.load('auth2', () => {
      ...
      if (!GoogleAuth) {
        window.gapi.auth2.init(initParams).then(async res => {
          if (!wasUnmounted) {
  ...
  const signIn = e => {
    ...
    if (googleStatus.loaded) {
      ...
      if (responseType === 'code') {
        GoogleAuth.grantOfflineAccess(initParams).then(res => onSuccess(res), err => onFailure(err));
      } else {
        GoogleAuth.signIn(initParams).then(res => {
          setFormState({
            loading: false,
            result: {
              error: false
  ...
  const signOut = e => {
    ...
    if (googleStatus.loaded) {
      const GoogleAuth = window.gapi.auth2.getAuthInstance();
      GoogleAuth.signOut().then(GoogleAuth.disconnect().then(() => {
        setFormState({
          loading: false,
          result: {
            error: false
  ...
  const handleSigninSuccess = async res => {
    ...
    try {
      ...
      const response = await ordering.users().authGoogle({
