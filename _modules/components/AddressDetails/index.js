...
export const AddressDetails = props => {
  ...
  const formatUrlMethod = location => {
    ...
    const staticmapUrl = isMultiCheckout ? `https://maps.googleapis.com/maps/api/staticmap?size=${mapConfigs?.mapSize?.width || 500}x${mapConfigs?.mapSize?.height || 190}&scale=2&maptype=roadmap&markers=icon:%7Ccolor:red%7C${orderLocation?.lat},${orderLocation?.lng}${businessesMarkers}&key=${GM_API_KEY}` : `https://maps.googleapis.com/maps/api/staticmap?size=${mapConfigs?.mapSize?.width || 500}x${mapConfigs?.mapSize?.height || 190}&scale=2&maptype=roadmap&markers=icon:%7Ccolor:red%7C|${orderLocation?.lat},${orderLocation?.lng}&markers=icon:${logo ? optimizeImage(logo, 'w_60,h_60,r_max') : '%7Ccolor:white%7C'}|${location?.lat},${location?.lng}&key=${GM_API_KEY}`;
    return orderState.options.type === 1 ? `https://maps.googleapis.com/maps/api/staticmap?size=${mapConfigs?.mapSize?.width || 500}x${mapConfigs?.mapSize?.height || 190}&center=${orderLocation?.lat},${orderLocation?.lng}&zoom=${mapConfigs?.mapZoom || 15}&scale=2&maptype=roadmap&markers=icon:%7Ccolor:red%7C${orderLocation?.lat},${orderLocation?.lng}&key=${GM_API_KEY}` : `${staticmapUrl}`;
  };
  /**
  ...
  const getBusiness = async () => {
    try {
      ...
      } = await ordering.businesses(businessId).select(['location', 'logo']).get({
        cancelToken: source
      });
      setLogo(result.logo);
