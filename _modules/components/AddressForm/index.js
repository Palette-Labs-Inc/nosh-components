...
export const AddressForm = props => {
  ...
  const loadAddress = async (userId, addressId) => {
    try {
      ...
      const {
        content
      } = await ordering.users(userId).addresses(addressId).get({
        accessToken,
        cancelToken: source
      });
      setAddressState({
  ...
  const showField = fieldName => {
    return !useValidationFileds || !validationFields.loading && !validationFields.fields?.address?.[fieldName] || !validationFields.loading && validationFields.fields?.address?.[fieldName] && validationFields.fields?.address?.[fieldName]?.enabled;
  };
  ...
  const isRequiredField = fieldName => {
    return useValidationFileds && !validationFields.loading && validationFields.fields?.address?.[fieldName] && validationFields.fields?.address?.[fieldName]?.enabled && validationFields.fields?.address?.[fieldName]?.required;
  };
  ...
  const saveAddress = async (values, userCustomerSetup) => {
    ...
    try {
      const {
        content
      } = await ordering.users(userId).addresses(addressState.address?.id).save({
        ...values,
        ...formState.changes
      }, {
        accessToken
