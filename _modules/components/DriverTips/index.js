  ...
  const handlerChangeOption = (driverTip, isFixedPrice = props.isFixedPrice) => {
    ...
    if (useOrderContext) {
      if (businessIds) {
        ...
        const tipPerCart = !isFixedPrice ? driverTip : parseFloat((Math.trunc(tip.dividedBy(businessIds?.length) * 100) / 100).toFixed(2));
        const correctionValue = !isFixedPrice ? 0 : parseFloat(tip.minus(new Decimal(tipPerCart).times(businessIds?.length)).toFixed(2));
        const tipsPerCart = businessIds.map((bid, idx) => {
          return {
            bid,
            value: parseFloat(new Decimal(tipPerCart).plus(idx === 0 ? correctionValue : 0).toFixed(2))
          };
        });
