...
export const OrderList = props => {
  ...
  const loadOrders = async (isNextPage, searchByOtherStatus, keepOrders = false, getFirstOrder) => {
    if (pagination?.currentPage === pagination?.totalPages && pagination?.total !== null && !getFirstOrder) {
      return;
    }
    ...
    try {
      ...
      if (searchByOtherStatus) {
        setOrderList({
          loading: false,
          orders: response.content.error ? [] : [...response.content.result, ...orderList.orders],
      ...
      setProfessionals([...response.content.result, ...orderList.orders].reduce((previousValue, currentValue) => previousValue.concat(currentValue?.products[0]?.calendar_event?.professional), []).filter((professional, i, hash) => professional && hash.map(_professional => _professional?.id).indexOf(professional?.id) === i));
      ...
      setProducts([...response.content.result, ...orderList.orders].filter(order => !businessesSearchList || businessesSearchList?.businesses?.some(business => order?.business_id === business?.id)).map(order => order.products.map(product => ({
        ...product,
        business: order?.business,
        businessId: order?.business_id
      }))).flat().filter((product, i, hash) => hash.map(_product => _product?.product_id).indexOf(product?.product_id) === i));
      if (isBusiness) {
