...
export const AddressList = props => {
  ...
  const loadAddresses = async () => {
    try {
      ...
      const {
        content
      } = await ordering.setAccessToken(accessToken).users(userId).addresses().get({
        cancelToken: source
      });
      setAddressList({
        loading: false,
  ...
  const handleSetDefault = async (address, userCustomerSetup, sameAddress, dontSetUserCustomer) => {
    ...
    try {
      ...
      const {
        content
      } = await ordering.setAccessToken(accessToken).users(userId).addresses(address.id).save({
        default: true
      });
      setActionStatus({
        loading: false,
  ...
  const handleDelete = async address => {
    ...
    try {
      ...
      const {
        content
      } = await ordering.users(userId).addresses(address.id).delete({
        accessToken
      });
      setActionStatus({
        loading: false,
