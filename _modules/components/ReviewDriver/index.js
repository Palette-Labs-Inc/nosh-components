...
export const ReviewDriver = props => {
  ...
  const handleSendDriverReview = async () => {
    ...
    try {
      const userId = isProfessional ? order?.products[0]?.calendar_event?.professional?.id : order?.driver?.id;
      ...
      if (!error) {
        ...
        if (isToast) {
          showToast(ToastType.Success, isProfessional ? t('PROFESSIONAL_REVIEW_SUCCESS_CONTENT', 'Thank you, Professional review successfully submitted!') : t('DRIVER_REVIEW_SUCCESS_CONTENT', 'Thank you, Driver review successfully submitted!'));
        }
      } else {
        setFormState({
          ...formState,
