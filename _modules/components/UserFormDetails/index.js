...
export const UserFormDetails = props => {
  ...
  const [removeAccountState, setAccountState] = useState({
    loading: false,
    error: null,
    result: null
  });
   ...
   * Clean formState
   */
  const cleanFormState = values => setFormState({
    ...formState,
    ...values
  });
  ...
  const handleUpdateClick = async (changes, isImage, image) => {
    ...
    try {
      ...
      if (isImage) {
        response = await ordering.users(props?.userData?.id || userState.result.result.id).save({
  ...
  const showField = fieldName => {
    return !useValidationFields || !validationFields.loading && !validationFields.fields?.checkout?.[fieldName] || !validationFields.loading && validationFields.fields?.checkout?.[fieldName] && validationFields.fields?.checkout?.[fieldName]?.enabled;
  };
  ...
  const isRequiredField = fieldName => {
    return useValidationFields && !validationFields.loading && validationFields.fields?.checkout?.[fieldName] && validationFields.fields?.checkout?.[fieldName]?.enabled && validationFields.fields?.checkout?.[fieldName]?.required;
  };
  const handleToggleAvalaibleStatusDriver = async newValue => {
    try {
      ...
      const response = await ordering.users(props?.userData?.id || userState.result.result.id).save({
        available: newValue
      }, {
        accessToken: accessToken
      });
      ...
      changes: {
        ...
        settings: {
          email: {
            ...
            promotions: value?.email
          },
          notification: {
            newsletter: value?.notification,
            promotions: value?.notification
          },
          sms: {
            newsletter: value?.sms,
            promotions: value?.sms
          }
        }
      }
    });
  ...
  useEffect(() => {
    updatePromotions(singleNotifications?.loading ? singleNotifications?.changes : notificationsGroup?.changes, singleNotifications?.loading ? setSingleNotifications : setNotificationsGroup, singleNotifications?.loading ? singleNotifications : notificationsGroup);
