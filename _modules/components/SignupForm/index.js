...
export const SignupForm = props => {
  ...
  const handleSignupClick = async values => {
    ...
    try {
      ...
      const response = await ordering.users().save(newData, {
        cancelToken: source
      });
      setFormState({
  ...
  const showField = fieldName => {
    return !useChekoutFileds || !validationFields.loading && !validationFields.fields?.checkout?.[fieldName] || !validationFields.loading && validationFields.fields?.checkout?.[fieldName] && validationFields.fields?.checkout?.[fieldName]?.enabled;
  };
  ...
  const isRequiredField = fieldName => {
    return fieldName === 'password' || useChekoutFileds && !validationFields.loading && validationFields.fields?.checkout?.[fieldName] && validationFields.fields?.checkout?.[fieldName]?.enabled && validationFields.fields?.checkout?.[fieldName]?.required;
  };
  ...
  */
  ...
  useEffect(() => {
    setIsReCaptchaEnable(props.isRecaptchaEnable && configs && Object.keys(configs).length > 0 && configs?.security_recaptcha_signup?.value === '1');
  }, [configs]);
  useEffect(() => {
