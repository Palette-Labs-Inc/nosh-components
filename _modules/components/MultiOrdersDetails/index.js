...
export const MultiOrdersDetails = props => {
  ...
  const getOrders = async () => {
    try {
      ...
      } = await ordering.setAccessToken(token).orders().get(options);
      setOrdersList({
        loading: false,
        orders: error ? [] : result,
      ...
      if (!error) {
        const _total = result.reduce((total, order) => {
          return total + order?.summary?.total;
        }, 0);
        const _subtotal = result.reduce((subtotal, order) => {
          return subtotal + order?.summary?.subtotal;
        }, 0);
        const _tax = result.reduce((tax, order) => {
          return tax + order?.summary?.tax;
        }, 0);
        setOrdersSummary({
          total: _total,
          subtotal: _subtotal,
