...
export const PaymentOptionStripeRedirect = props => {
  ...
  const modalName = () => {
    ...
    paymentMethods.map((paym, i) => {
      i === paymentMethods.length - 1 ? name += `${paym.name}` : name += `${paym.name}, `;
    });
    return name;
  };
  ...
  const getCredentials = async () => {
    ...
    const response = await fetch(`${ordering.root}/payments/stripe/credentials`, {
      headers: {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, UIComponent && stripePK && /*#__PURE__*/React.createElement(UIComponent, _extends({}, props, {
    stripePK: stripePK,
    modalName: modalName()
  })));
};
