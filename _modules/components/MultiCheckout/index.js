...
export const MultiCheckout = props => {
  ...
  */
  ...
  const totalCartsPrice = openCarts?.length && openCarts.reduce((total, cart) => {
    return total + cart?.total;
  }, 0);
  const totalCartsFee = openCarts?.length && openCarts?.filter(cart => cart?.status !== 1 && cart?.valid)?.reduce((total, cart) => {
    return total + cart?.delivery_price_with_discount;
  }, 0);
  const handleGroupPlaceOrder = async confirmPayment => {
    ...
    const {
      error,
      result
    } = await placeMultiCarts(payload, cartUuid);
    ...
    if (confirmPayment && paymethodSelected?.gateway === 'global_apple_pay') {
      ...
      if (paymentEvent?.data?.extra?.client_secret) {
        ...
        } = await confirmPayment(paymentEvent?.data?.extra?.client_secret);
  ...
  const handleSelectWallet = async (isChecked, wallet) => {
    ...
    const url = isChecked ? `${ordering.root}/cart_groups/${cartGroup?.result?.uuid}/wallets` : `${ordering.root}/cart_groups/${cartGroup?.result?.uuid}/wallets/${wallet.id}`;
    try {
