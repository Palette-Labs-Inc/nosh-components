...
export const SingleOrderCard = props => {
  ...
  const handleFavoriteOrder = async (isAdd = false) => {
    ...
    try {
      ...
      const fetchEndPoint = isAdd ? `${ordering.root}/users/${user?.id}/favorite_orders` : `${ordering.root}/users/${user.id}/favorite_orders/${order?.id}`;
      ...
      if (!content.error) {
        ...
        handleUpdateOrderList && handleUpdateOrderList(order?.id, {
          favorite: isAdd
        });
  ...
  const handleRemoveCart = async _order => {
    ...
    const uuids = _businessIds.map((b, index) => ({
      uuid: carts[`businessId:${b}`]?.uuid,
      orderIdx: index
    })) ?? [];
