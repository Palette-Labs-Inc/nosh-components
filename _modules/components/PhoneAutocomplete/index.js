...
export const PhoneAutocomplete = props => {
  ...
  const [customersPhones, setCustomersPhones] = useState({
    users: [],
    loading: false,
    error: null
  });
  ...
  const getUsers = async () => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(token).users().where(conditions).get();
      setCustomersPhones({
        ...customersPhones,
        users: result,
        loading: false
  ...
  const getBusiness = async () => {
    ...
    } = await ordering.businesses('mcbonalds').select(['address', 'location', 'distance']).parameters().get();
    if (error) {
      setAlertState({
        open: true,
        content: result
  ...
  const setGuestOptions = async ({
    ...
    try {
      ...
      } = await ordering.users(userObj.id).addresses().get();
      if (error) {
        setAlertState({
          open: true,
      ...
      if (!addressSelected?.id) {
        const response = await ordering.users(userObj.id).addresses().save({
        ...
        const addressResponse = await ordering.users(userObj.id).addresses(addressSelected?.id).save({
          default: true
        });
  ...
  useEffect(() => {
    if (phone && phone.length >= 7 && (customersPhones?.users?.length === 0 || phone.length === 7)) {
      getUsers();
    }
