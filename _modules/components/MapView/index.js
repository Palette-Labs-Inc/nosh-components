...
export const MapView = props => {
  ...
  const getBusinessLocations = async () => {
    ...
    } = await ordering.setAccessToken(session.token).orders().asDashboard().get(options);
    if (!error) {
      result.map(order => {
  ...
  const setDriverLocation = async location => {
    try {
      await ordering.setAccessToken(session.token).users(session.user.id).driverLocations().save(location);
    } catch (error) {
