...
export const Contacts = props => {
  const {
    ...
    businessProps,
    customerProps,
    // don't use because it prevents fetch "qualification"
    driverProps,
    businessConditions,
    customerConditions,
    driverConditions,
  ...
  const getOrders = async nextPage => {
    ...
    try {
      ...
      } = await ordering.orders().parameters(parameters).asDashboard().get({
        query: pageFetch
      });
      if (!error) {
        let hash = {};
  ...
  const getBusinesses = async nextPage => {
    ...
    try {
      ...
      } = await ordering.businesses().select(businessProps).parameters(params).where(where).asDashboard().get({
        query: pageFetch
      });
      if (!error) {
        setContacts({
  ...
  const getCustomers = async nextPage => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(token).users().where(where).get({
        query: pageFetch
      });
      if (!error) {
        setContacts({
  ...
  const getDrivers = async nextPage => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(token).users().select(driverProps).where(where).get({
        query: pageFetch
      });
      if (!error) {
        setContacts({
  ...
  const handleMessage = useCallback(async message => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(token).orders(orderId).asDashboard().get();
      if (!error) {
