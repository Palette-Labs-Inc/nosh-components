...
export const ProductsListing = props => {
  ...
  const getProducts = async () => {
    try {
      ...
      } = await ordering.businesses(businessId).products().parameters({
        type: orderState.options?.type || 1
      }).get();
      const productsFiltered = searchValue || categoryValue ? result.filter(product => isMatchSearch(product.name, product.description) && isMatchCategory(product.category_id)) : result;
      setProductsList({
        ...productsList,
        loading: false,
  ...
  const getCategories = async () => {
    ...
    } = await ordering.businesses(businessId).categories().get();
