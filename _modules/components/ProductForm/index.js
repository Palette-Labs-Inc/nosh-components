...
export const ProductForm = props => {
  ...
  */
  ...
  const [professionalListState, setProfessionalListState] = useState({
    loading: false,
    professionals: [],
    error: null
  });
  ...
  const handleFavoriteProduct = async (productFav, isAdd = false) => {
    ...
    try {
      ...
      const fetchEndpoint = isAdd ? `${ordering.root}/users/${user?.id}/favorite_products` : `${ordering.root}/users/${user.id}/favorite_products/${productId}`;
  ...
  const loadProductWithOptions = async () => {
    try {
      ...
      } = await ordering.businesses(props.businessId).categories(props.categoryId).products(props.productId).get({
        cancelToken: source
      });
      if (!error) {
        setProduct({
  ...
  const checkErrors = () => {
    ...
    product.product?.options?.forEach(option => {
      ...
      const quantity = suboptions ? option.limit_suboptions_by_max ? Object.values(suboptions).reduce((count, suboption) => {
        return count + suboption.quantity;
  ...
  const getProfessionalList = async () => {
    try {
      ...
      } = await ordering.businesses(props.businessId).select(['id', 'professionals']).get();
      if (!error) {
        setProfessionalListState({
          ...professionalListState,
          loading: false,
  ...
  useEffect(() => {
    if (product?.product && product.product?.options?.length > 0) {
      const options = [].concat(...product.product.options.filter(option => {
        const preselected = checkHasPreselected(options, option);
        return (option.min === 1 && option.max === 1 && option.suboptions.filter(suboption => suboption.enabled).length === 1 || option.suboptions.filter(suboption => suboption.preselected).length > 0) && (!option?.conditioned || option?.conditioned && preselected);
      }));
      if (!options?.length) {
        return;
      }
      const suboptions = [].concat(...options.map(option => option.suboptions)).filter(suboption => suboption.enabled);
      const states = suboptions.map((suboption, i) => {
        const price = options[i]?.with_half_option && suboption?.half_price && suboption?.position !== 'whole' ? suboption.half_price : suboption.price;
        return {
          id: suboption.id,
          name: suboption.name,
      ...
      let suboptionsArray = [];
      options.map(option => {
        const defaultSuboptions = option.suboptions.filter(suboption => suboption?.enabled && (suboption?.preselected || option?.suboptions?.length === 1)).map(suboption => {
          return {
            option: option,
            suboption: suboption,
  ...
  if (isStarbucks) {
    useEffect(() => {
      if (product?.product && Object.keys(product?.product).length) {
        const options = [].concat(...product.product.options.filter(option => option.name === 'Tamaño' && option.suboptions.filter(suboption => suboption.name === 'Grande (16oz - 437ml)').length === 1));
        if (!options?.length) {
          return;
        }
        const suboptions = [].concat(...options.map(option => option.suboptions)).filter(suboption => suboption.name === 'Grande (16oz - 437ml)');
        const states = suboptions.map((suboption, i) => {
          const price = options[i].with_half_option && suboption.half_price && suboption?.position !== 'whole' ? suboption.half_price : suboption.price;
          return {
            id: suboption.id,
