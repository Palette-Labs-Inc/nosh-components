...
export const Checkout = props => {
   ...
   */
  const [instructionsOptions, setInstructionsOptions] = useState({
    loading: false,
    result: [{
      id: null,
      enabled: true,
  ...
  */
   ...
   * Loyalty plans state
   */
  const [loyaltyPlansState, setLoyaltyPlansState] = useState({
    loading: true,
    error: null,
    result: []
  });
  const businessId = props.uuid ? Object.values(orderState.carts).find(_cart => _cart?.uuid === props.uuid)?.business_id ?? {} : props.businessId;
  /**
   * Current cart
   */
  ...
  const getBusiness = async () => {
    ...
    try {
      ...
      } = await ordering.businesses(businessId).select(propsToFetch).parameters(parameters).get();
  ...
  const handlerClickPlaceOrder = async (paymentOptions, payloadProps, confirmPayment) => {
    ...
    if (confirmPayment && result?.result?.paymethod_data?.gateway === 'apple_pay') {
      ...
      } = await confirmPayment(result?.result?.paymethod_data?.result?.client_secret);
      if (confirmApplePayError) {
        setErrors(confirmApplePayError);
      }
    }
    if (paymethodsWithoutSaveCard.includes(cartResult?.paymethod_data?.gateway) && cartResult?.paymethod_data?.result?.hash && cartResult?.paymethod_data?.status === 2 && !payloadProps.isNative) {
   ...
   */
  const handleChangeSpot = async ({
    isCheckout = true,
    bodyToSend
  }) => {
    try {
      const id = isCheckout ? cart?.uuid : cart?.id;
      const endpointToFetch = isCheckout ? ordering.setAccessToken(token).carts(id).set(bodyToSend) : ordering.setAccessToken(token).orders(id).save(bodyToSend);
      const {
        content: {
          error,
          result
        }
      } = await endpointToFetch;
      if (!error && !isApp) {
        onRemoveSpotNumber && onRemoveSpotNumber(cart?.business?.slug);
      }
      showToast(error ? ToastType.Error : ToastType.Success, error ? t('ERROR', result[0]) : t('SPOT_CHANGE_SUCCESS_CONTENT', 'Changes applied correctly'));
    } catch (err) {
      console.log(err);
    }
  };
    ...
    try {
      ...
      if (!error) {
        setInstructionsOptions({
          loading: false,
          result: [...instructionsOptions.result, ...result]
        });
        return;
      }
      setInstructionsOptions({
  ...
  useEffect(() => {
    Promise.all([getDeliveryOptions(), getLoyaltyPlans()].map(promise => {
