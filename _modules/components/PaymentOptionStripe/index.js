...
export const PaymentOptionStripe = props => {
  ...
  const [defaultCardSetActionStatus, setDefaultCardSetActionStatus] = useState({
    loading: false,
    error: null
  });
  const requestState = {};
  ...
  const getCards = async () => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(token).paymentCards(businessId, user.id).get({
        cancelToken: source
      });
  ...
  const deleteCard = async card => {
    try {
      ...
      } = await ordering.paymentCards(-1, user.id, card.id).delete();
      if (!error) {
