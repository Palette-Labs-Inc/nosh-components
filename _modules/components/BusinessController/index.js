...
export const BusinessController = props => {
  ...
  const getBusiness = async () => {
    ...
    try {
      ...
      } = await ordering.businesses(businessId).select(businessAttributes).get();
      if (!error) {
        setBusinessState({
          ...businessState,
          business: result,
  ...
  const handleFavoriteBusiness = async (isAdd = false) => {
    ...
    try {
      ...
      const changes = {
        object_id: businessState?.business?.id
      };
      ...
      const fetchEndpoint = isAdd ? `${ordering.root}/users/${user?.id}/favorite_businesses` : `${ordering.root}/users/${user.id}/favorite_businesses/${businessState?.business?.id}`;
      ...
      if (!content.error) {
        ...
        handleUpdateBusinessList && handleUpdateBusinessList(businessState?.business?.id, {
          favorite: isAdd
        });
        if (favoriteIds) {
          const updateIds = isAdd ? [...favoriteIds, businessState?.business?.id] : favoriteIds.filter(item => item !== businessState?.business?.id);
        ...
        props.handleCustomUpdate && props.handleCustomUpdate(businessState?.business?.id, {
          favorite: isAdd
        });
  ...
  const updateBusiness = async (businessId, updateParams = {}) => {
    ...
    try {
      ...
      } = await ordering.businesses(businessId).save(updateParams);
      if (!error) {
        setBusinessState({
          ...businessState,
          business: result,
