...
export const BusinessProductsListing = props => {
  ...
  const [formTaxState, setFormTaxState] = useState({
    loading: false,
    changes: {},
    result: {
      error: false
  ...
  const [formFeeState, setFormFeeState] = useState({
    loading: false,
    changes: {},
    result: {
      error: false
  ...
  const getProducts = async (newFetch, currentPage, currentPageSize) => {
    if (!businessState?.business?.lazy_load_products_recommended) {
      ...
      if (categorySelected) {
        ...
        let productsFiltered = categoryFiltered?.products?.filter(product => isMatchSearch(product.name, product.description));
        if (!productsFiltered) {
          productsFiltered = categorySelected?.products?.filter(product => isMatchSearch(product.name, product.description));
        }
        categoryState.products = productsFiltered || [];
      } else {
        const productsFiltered = businessState?.business?.categories?.reduce((products, category) => {
          if (category.products) {
            return [...products, ...category.products];
          }
          return [...products];
        }, []).filter(product => isMatchSearch(product.name, product.description));
        categoryState.products = productsFiltered || [];
      }
      setCategoryState({
        ...categoryState,
    ...
    try {
      const functionFetch = categorySelected?.id && categorySelected?.id !== 'featured' ? ordering.businesses(businessState.business?.id).categories(categorySelected?.id).products() : ordering.businesses(businessState.business?.id).products();
      const source = {};
      requestsState.products = source;
      setRequestsState({
        ...requestsState
      });
      const productEndpoint = where.conditions.length > 0 ? functionFetch.parameters(parameters).where(where) : functionFetch.parameters(parameters);
      const {
        content: {
          error,
          result,
  ...
  const getProduct = async () => {
    if (categoryId && productId && businessState.business?.id) {
      try {
        ...
        } = await ordering.businesses(businessState.business?.id).categories(categoryId).products(productId).get({
          cancelToken: source
        });
  ...
  const getBusiness = async () => {
    try {
      ...
      } = await ordering.businesses(businessSlug).asDashboard().get();
  ...
  const handleUpdateTaxesProducts = ({
    tax,
    isRemove,
    id
  }) => {
  ...
  const handleUpdateFeesProducts = ({
    fee,
    isRemove
  }) => {
    const _categories = [...businessState?.business?.categories];
