...
export const ExportCSV = props => {
  ...
  const getCSV = async filterApply => {
    ...
    try {
      ...
      const functionFetch = filterApply ? `${ordering.root}/orders.csv?mode=dashboard&orderBy=id&where=${JSON.stringify(filterConditons)}` : `${ordering.root}/orders.csv?mode=dashboard&orderBy=id`;
