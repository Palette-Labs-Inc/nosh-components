...
export const SettingsList = props => {
  ...
  const saveChanges = (changeValue, id) => {
    ...
    if (found) {
      _configs = formState?.changes.map(config => config.id === id ? {
        ...config,
        value: changeValue
      } : config);
    } else {
    ...
    const defaultConfigs = configs.map(config => config.id === id ? {
      ...config,
      value: changeValue
    } : config);
    setFormState({
  ...
  const saveConfig = async (id, params) => {
    ...
    try {
      ...
      } = await ordering.configs(id).save(params);
      if (!error) {
  ...
  useEffect(() => {
    if (formState?.API && formState?.changes?.length > 0) {
      const params = {
        key: formState?.changes[0].key,
        value: formState?.changes[0].value
      };
