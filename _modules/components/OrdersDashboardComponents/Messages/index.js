...
export const Messages = props => {
    ...
    try {
      ...
      if (!error) {
        if (setOrderMessages && orderMessages) {
          setOrderMessages({
            ...orderMessages,
            messages: [...orderMessages.messages, result]
          });
        } else {
          setMessages({
            ...messages,
  ...
  const loadMessages = async () => {
    try {
      ...
      const functionFetch = asDashboard ? `${ordering.root}/orders/${orderId}/messages?mode=dashboard` : `${ordering.root}/orders/${orderId}/messages`;
