...
export const DriversList = props => {
  ...
  const handleAssignDriver = async assign => {
    try {
      ...
      const {
        content
      } = await ordering.setAccessToken(session.token).orders(assign.orderId).save({
  ...
  const getDrivers = async () => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).users().select(propsToFetch).where(where).get({
        cancelToken: source
      });
      setDriversList({
        ...driversList,
