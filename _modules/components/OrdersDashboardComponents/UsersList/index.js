...
export const UsersList = props => {
  ...
  const [deleteUsersActionState, setDeleteUsersActionState] = useState({
    loading: false,
    error: null
  });
  const [occupationsState, setOccupationsState] = useState({
    occupations: [],
    loading: false,
    error: null
  });
  ...
  const getUsers = async (page, pageSize) => {
    try {
      ...
      const fetchEndpoint = where ? ordering.setAccessToken(session.token).users().select(propsToFetch).parameters(parameters).where(where) : ordering.setAccessToken(session.token).users().select(propsToFetch).parameters(parameters);
      const {
        content: {
          result,
          pagination
  ...
  const handleChangeUserType = async user => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).users(user.id).save({
        level: user.level
      }, {
        cancelToken: source
      });
  ...
  const handleDeleteUser = async userId => {
    try {
      ...
      const {
        content
      } = await ordering.setAccessToken(session.token).users(userId).delete();
      if (!content.error) {
