...
export const DashboardOrdersList = props => {
  ...
  const handleUpdateOrderStatus = async order => {
    try {
      ...
      const {
        content
      } = await ordering.setAccessToken(accessToken).orders(order?.id).save({
  ...
  */
  const handleDrop = (event, columnName) => {
    ...
    const _allowColumnsUpdated = {
      ...allowColumns,
      [transferColumnName]: {
        ...allowColumns[transferColumnName],
  ...
  const saveUserSettings = async allowColumnsUpdated => {
    try {
      ...
      const _allowColumnsUpdated = {
        ...allowColumnsUpdated,
        timer: {
          ...allowColumnsUpdated?.timer,
          visable: false
        }
      };
      await ordering.users(session?.user?.id).save({
        settings: {
          ..._settings,
    ...
    const getUser = async () => {
      try {
        ...
        setAllowColumns({
          ...allowColumnsModel,
          slaBar: {
            ...allowColumnsModel?.slaBar,
            visable: configState?.configs?.order_deadlines_enabled?.value === '1'
          },
          timer: {
            ...allowColumnsModel?.timer,
            visable: configState?.configs?.order_deadlines_enabled?.value === '1'
          }
        });
      } catch (err) {
