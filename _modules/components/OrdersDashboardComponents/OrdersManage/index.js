...
export const OrdersManage = props => {
   ...
   */
  const [driverGroupList, setDriverGroupList] = useState({
    groups: [],
    loading: false,
    error: null
  });
  ...
  const deleteOrder = async id => {
    try {
      ...
      const {
        content
      } = await ordering.setAccessToken(token).orders(id).delete({
        cancelToken: source
      });
      if (!content.error) {
  ...
  const getDrivers = async () => {
    try {
      ...
      } = await ordering.setAccessToken(token).users().select(driversPropsToFetch).where([{
        attribute: 'level',
        value: [4]
      }]).get({
        cancelToken: source
