...
export const BusinessList = props => {
  ...
  const getBusinesses = async (newFetch, specificPagination, prev) => {
    try {
      ...
      let parameters = asDashboard ? {} : {
        location: !customLocation ? `${orderState.options?.address?.location?.lat},${orderState.options?.address?.location?.lng}` : `${customLocation.lat},${customLocation.lng}`,
      ...
      if (priceLevelSelected || initialPricelevel) {
        conditions.push({
          ...
          value: initialPricelevel ?? priceLevelSelected
        });
      }
      if (businessId) {
        conditions.push({
      ...
      const {
        ...
        cancelToken: source,
        advancedSearch: advancedSearchEnabled && searchValue?.length >= 3
      });
      if (!error) {
        if (isSortByReview) {
  ...
  const getFranchise = async () => {
    try {
      ...
      if (result?.enabled) setFranchiseEnabled(result?.enabled);else setBusinessesList({
        ...businessesList,
        loading: false
      });
    } catch (err) {
  ...
  }, [JSON.stringify(orderState.options), ordering?.countryCode, orderState.loading, businessTypeSelected, priceLevelSelected, searchValue, initialPricelevel, initialBuisnessType, timeLimitValue, orderByValue, maxDeliveryFee, businessId]);
  useEffect(() => {
