  ...
  const refreshConfigs = async (newCountryCode, configs = null) => {
    try {
      ...
      if (!configs) {
        const {
          content
        } = await ordering.configs().asDictionary().get(options);
        error = content.error;
      ...
      const conditionalConfigs = {
        dates_moment_format: {
          ...
          value: result?.dates_moment_format?.value || (result?.format_time?.value === '24' ? 'MM/DD HH:mm' : 'MM/DD hh:mma')
        },
        dates_general_format: {
          key: 'dates_general_format',
          value: result?.dates_general_format?.value || (result?.format_time?.value === '24' ? 'YYYY-MM-DD HH:mm:ss' : 'YYYY-MM-DD hh:mm:ssa')
        }
      };
      const configsResult = {
        ...customConfigs,
