...
export const OrderProvider = ({
  ...
  const [confirmAlert, setConfirm] = useState({
    show: false
  });
  ...
  const refreshOrderOptions = async () => {
    try {
      ...
      const countryCode = countryCodeFromLocalStorage && countryCodeFromLocalStorage !== state?.options?.address?.country_code ? countryCodeFromLocalStorage : state?.options?.address?.country_code;
      if (countryCode) {
        options.headers = {
      ...
      if (localOptions) {
        ...
        if (Object.keys(localOptions.address).length > 0) {
          ...
          if (!address) {
            ...
            } = await ordering.setAccessToken(session.token).users(userId).addresses().save(localOptions.address);
            if (!error) {
              address = result;
            }
          } else {
            await ordering.setAccessToken(session.token).users(userId).addresses(address.id).save({
              default: true
            });
          }
      ...
      const message = err?.message?.includes('Internal error') ? 'INTERNAL_ERROR' : err.message;
      setAlert({
        show: true,
        content: [message]
      });
  ...
  const checkAddress = address => {
    ...
    props.forEach(prop => {
      if (state.options?.address && state.options?.address?.[prop]) {
        if (prop === 'location') {
          values.push(address?.[prop]?.lat === state.options?.address?.[prop]?.lat && address?.[prop]?.lng === state.options?.address?.[prop]?.lng);
        } else {
  ...
  const updateOrderOptions = async changes => {
    if (session.auth) {
      ...
      try {
        ...
        const countryCode = changes?.country_code && changes?.country_code !== state?.options?.address?.country_code ? changes?.country_code : countryCodeFromLocalStorage ?? changes?.country_code ?? state?.options?.address?.country_code;
        if (countryCode) {
          options.headers = {
            ...options.headers,
            'X-Country-Code-X': countryCode
        ...
        } = await ordering.setAccessToken(session.token).orderOptions().save(body, options);
        if (!error) {
          const {
            carts,
            ...options
  ...
  const addProduct = async (product, cart, isQuickAddProduct, isPlatformProduct = false) => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).carts().addProduct(body, {
        headers: {
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
        }
  ...
  const removeProduct = async (product, cart) => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).carts().removeProduct(body, {
        headers: {
          'X-App-X': ordering.appId,
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
  ...
  const updateProduct = async (product, cart, isQuickAddProduct) => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).carts().updateProduct(body, {
        headers: {
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
        }
    ...
    try {
      ...
      if (customParams && isAlsea) {
        ...
        if (result.message !== 'Cup\u00f3n v\u00e1lido') {
          setAlert({
            show: true,
            content: result.message === 'Not found' ? ['ERROR_INVALID_COUPON'] : [result.message]
          });
          setState({
            ...state,
            loading: false
      ...
      const {
        content
      } = await ordering.setAccessToken(session.token).carts().applyCoupon(body, {
        headers: {
          'X-App-X': ordering.appId,
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
  ...
  const changeDriverTip = async (businessId, driverTipRate = 0, isFixedPrice) => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(session.token).carts().changeDriverTip(body, {
        headers: {
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
        }
  ...
  const changePaymethod = async (businessId, paymethodId, paymethodData) => {
    ...
    try {
      ...
      } = await ordering.setAccessToken(session.token).carts().changePaymethod(body, {
        headers: {
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
        }
  ...
  const placeCart = async (cardId, data) => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).carts(cardId).place(body, {
        headers: {
          'X-Socket-Id-X': socket?.getId(),
          'X-Country-Code-X': countryCode
        }
  ...
  */
  ...
  const reorder = async (orderId, offAlert) => {
    try {
      ...
      } = await ordering.setAccessToken(session.token).orders(orderId).reorder(options);
      if (!error) {
        state.carts[`businessId:${result.business_id}`] = result;
  ...
  useEffect(() => {
    if (configTypes?.length > 0 && state.options.type && !configTypes.includes(state.options.type)) {
      ...
      updateOrderOptions(validDefaultValue ? {
        type: configState?.configs?.default_order_type?.type
      } : {
        type: configTypes[0]
      });
      setAlert({
